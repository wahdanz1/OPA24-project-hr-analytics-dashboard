...
def open_chat():
    ...
    if "query" not in st.session_state:
        st.session_state.query = "" # Initialize query string to empty
    ...
    with col2:
        ...
        if not st.session_state.df.empty: # Check if DataFrame is actually not empty
            graph_tab, data_tab, sql_tab = st.tabs(["Graph Preview", "Data Preview", "SQL Query"])
            with graph_tab:
            ...
            with sql_tab:
                ...
                st.write("### SQL Query")
                st.code(st.session_state.query, language="sql", wrap_lines=True) # The SQL query generated by Gemini
    ...
    if prompt:
        ...
        # ---------- GENERATING & STORING DATAFRAME / QUERY ----------
        ...
        if response and response.get("query"): # Ensure response is not None and has 'query' key
            try:
                ...
                st.session_state.query = response["query"] # Store the SQL query in session state
